		...
		<view class="goods-list">
            ...
            <view class="row" v-for="(row,index) in goodsList" :key="index" >
				...
				<view class="menu" @tap.stop="deleteGoods(row.goodsid)">
					<view class="icon shanchu">点击删除</view>
				</view>
				<!-- 商品 -->
					...
					<view class="goods-info" @tap="toGoods(row)">
						...
						<view class="info">
							...
							<view class="price-number">
								<view class="price">￥{{row.goodsprice}}</view>
								<view class="number">
									<view class="sub" @tap.stop="sub(index)">
										<view class="icon jian">-</view>
									</view>
									<view class="input" @tap.stop="discard">
										<input type="number" v-model="row.goodsnum" @input="sum($event,index)" />
									</view>
	...
	export default {
		data() {
			return {
				...
				userid:'',
				goodsList:[
					// {id:1,img:'/static/img/goods/p1.jpg',name:'商品1',price:127.5,number:1,selected:false},
					// {id:2,img:'/static/img/goods/p2.jpg',name:'商品2',price:127.5,number:1,selected:false},
					// {id:3,img:'/static/img/goods/p3.jpg',name:'商品3',price:127.5,number:1,selected:false},
					// {id:4,img:'/static/img/goods/p4.jpg',name:'商品4',price:127.5,number:1,selected:false},
					// {id:5,img:'/static/img/goods/p5.jpg',name:'商品5',price:127.5,number:1,selected:false}
				],
				//控制滑动效果
				theIndex:null,
				oldIndex:null,
		...
		onShow() {
			let self = this;
			this.$axios.post('/shoppingcar/showShoppingcar',{
				userid:this.userid,
			}).then(rep=>{
		...
		methods: {
			...
			//删除商品
			deleteGoods(id){
				console.log("删除商品传递的id："+id)
				let len = this.goodsList.length;
				for(let i=0;i<len;i++){
					if(id==this.goodsList[i].goodsid){
			...
			// 减少数量
			sub(index){
				if(this.goodsList[index].goodsnum<=1){
					return;
				}
			...
			sum(e,index){
				...
				for(let i=0;i<len;i++){
					if(this.goodsList[i].selected) {
						if(e && i==index){
							this.sumPrice = this.sumPrice + (e.detail.value*this.goodsList[i].goodsprice);
						}else{
							this.sumPrice = this.sumPrice + (this.goodsList[i].goodsnum*this.goodsList[i].goodsprice);
						}
					}
				}
